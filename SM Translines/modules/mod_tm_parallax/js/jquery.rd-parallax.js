  ...
  RDParallax.prototype = {
    ...
    createDOM: function () {
      ...
      parallax.$wrap
        ...
        .wrapInner($("<div/>", { class: parallax.settings.contentClass }))
        .prepend(
          $("<div/>", {
            class: parallax.options.pattern
              ? parallax.settings.patternClass
              : parallax.settings.imageClass,
          }).css({
            "background-image": "url(" + parallax.options.url + ")",
            "background-color": parallax.options.color,
          })
        );
      parallax.$image = parallax.options.pattern
        ? parallax.$wrap.find("." + parallax.settings.patternClass)
        : parallax.$wrap.find("." + parallax.settings.imageClass);
    },
    ...
    resize: function () {
      ...
      if (h < parallax.$wrap.outerHeight()) {
        h =
          parallax.$wrap.outerHeight() +
          $(window).height() * parallax.options.speed;
      }
    ...
    blur: function () {
      var parallax = this;
      if (
        parallax.options.blur &&
        !parallax.isIE() &&
        !parallax.options.pattern
      ) {
    ...
    isVisible: function () {
      ...
      return (
        parallaxOffset + parallaxHeight >= windowScroll &&
        parallaxOffset <= windowScroll + windowHeight
      );
    },
    isIE: function () {
