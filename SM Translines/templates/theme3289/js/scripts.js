...
jQuery(document).ready(function ($) {
      ...
      if ($(this).parent().not("span.checkbox")) {
        if (!$(this).attr("id")) {
          $(this)
            ...
            .after('<label class="checkbox_inner" for="checkbox' + i + '"/>');
        } else {
          $(this)
            .wrap('<span class="checkbox"/>')
            .after(
              '<label class="checkbox_inner" for="' + $(this).attr("id") + '"/>'
            );
        }
      }
    });
      ...
      if ($(this).parent().not("span.radio")) {
        if (!$(this).attr("id")) {
          $(this)
            ...
            .after('<label class="radio_inner" for="radio' + i + '"/>');
        } else {
          $(this)
            .wrap('<span class="radio"/>')
            .after(
              '<label class="radio_inner" for="' + $(this).attr("id") + '"/>'
            );
        }
      }
    });
  ...
  $(":header")
    .not("[class*=heading-style-]")
    .not(".alert-heading")
    .each(function () {
      $(this).addClass(
        "heading-style-" + $(this).prop("tagName").replace(/^\D+/g, "")
      );
    });
      ...
      .each(function (i) {
        ...
        if (a && z) {
          ...
          if ($('[class*="progress"]').length) {
            if (
              y.find('>.item_content [class*="progress"]').length != 0 &&
              !y.hasClass("animated")
            ) {
              animateProgress(y);
            }
  ...
  function animateProgress(el) {
    if (
      el.find(".radial-progress").length != 0 &&
      el.find(".radial-progress").hasClass("draw")
    ) {
...
(function ($) {
  function bottomFooter() {
    ...
    $("#wrapper").css({ "min-height": "100%", "padding-bottom": footerHeight });
    ...
    if (o.length) {
      ...
      $(document).ready(function () {
        o.each(function () {
          ...
          if (
            this.getAttribute("data-stroke") &&
            this.getAttribute("data-value") &&
            type
          ) {
            bar = new ProgressBar[type](this, {
              strokeWidth: Math.round(
                (parseFloat(this.getAttribute("data-stroke")) /
                  this.offsetWidth) *
                  100
              ),
              trailWidth: this.getAttribute("data-trail")
                ? Math.round(
                    (parseFloat(this.getAttribute("data-trail")) /
                      this.offsetWidth) *
                      100
                  )
                : 0,
              text: {
                value:
                  this.getAttribute("data-counter") === "true" ? "0" : null,
                ...
                .on(
                  ...
                  $.proxy(function () {
                    if (
                      ...
                      bar.animate(
                        parseInt(this.getAttribute("data-value")) / 100.0,
                        {
            ...
            console.error(
              this.className + ": progress bar type is not defined"
            );
          }
        });
      });
